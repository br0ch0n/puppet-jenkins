...
### Agents
You can automatically add agents to jenkins, and have them auto register themselves.  Most options are actually optional, as nodes will auto-discover the master, and connect.

Full documentation for the agent code is in jenkins::agent.

It requires the swarm plugin on the master & the class jenkins::agent on the agents, as below:
