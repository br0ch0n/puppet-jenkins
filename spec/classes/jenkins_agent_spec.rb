require 'spec_helper'

describe 'jenkins::agent' do

  shared_context 'a jenkins::agent catalog' do
    it do
      should contain_archive('get_swarm_client').with(
        :cleanup => false,
        :extract => false,
      )
    end
    it { should contain_file(agent_service_file) }
    it { should contain_service('jenkins-agent').with(:enable => true, :ensure => 'running') }
    # Let the different platform blocks define  `agent_runtime_file` separately below
    it { should contain_file(agent_runtime_file).with_content(/^FSROOT="\/home\/jenkins-agent"$/) }
    it { should contain_file(agent_runtime_file).without_content(/ -name /) }
    it { should contain_file(agent_runtime_file).with_content(/^AUTO_DISCOVERY_ADDRESS=""$/) }

    describe 'with manage_agent_user true and manage_client_jar enabled' do
      let(:params) { { :manage_agent_user => true, :manage_client_jar => true } }
      it { should contain_user('jenkins-agent_user').with_uid(nil).that_comes_before('Archive[get_swarm_client]') }
    end

    describe 'with manage_agent_user true and manage_client_jar false' do
      let(:params) { { :manage_agent_user => true, :manage_client_jar => false } }
      it { should contain_user('jenkins-agent_user').with_uid(nil) }
    end

    describe 'with ssl verification disabled' do
      let(:params) { { :disable_ssl_verification => true } }
      it { should contain_file(agent_runtime_file).with_content(/-disableSslVerification/) }
    end

    describe 'with auto discovery address' do
      let(:params) { { :autodiscoveryaddress => '255.255.255.0' } }
      it { should contain_file(agent_runtime_file).with_content(/^AUTO_DISCOVERY_ADDRESS="255.255.255.0"$/) }
    end

    describe 'agent_uid' do
      let(:params) { { :agent_uid => '123' } }
      it { should contain_user('jenkins-agent_user').with_uid(123) }
    end

    describe 'agent_groups' do
      let(:params) { { :agent_groups => 'docker' } }
      it { should contain_user('jenkins-agent_user').with_groups('docker') }
    end

    describe 'with a non-default $agent_home' do
      let(:home) { '/home/rspec-runner' }
      let(:params) { {:agent_home => home } }
      it { should contain_file(agent_runtime_file).with_content(/^FSROOT="#{home}"$/) }
    end

    describe 'with service disabled' do
      let(:params) { {:enable => false, :ensure => 'stopped' } }
      it { should contain_service('jenkins-agent').with(:enable => false, :ensure => 'stopped') }
    end

    describe 'with tool_locations' do
      let(:params) { { :tool_locations => 'Python-2.7:/usr/bin/python2.7 Java-1.8:/usr/bin/java' } }
      it do
        should contain_file(agent_runtime_file).
          with_content(/--toolLocation Python-2.7=\/usr\/bin\/python2.7/).
          with_content(/--toolLocation Java-1.8=\/usr\/bin\/java/)
      end
    end

    describe 'with a UI user/password' do
      let(:user) { '"frank"' }
      let(:password) { "abignale's" }
      let(:params) do
        {
          :ui_user => user,
          :ui_pass => password,
        }
      end

      it 'should escape the user' do
        should contain_file(agent_runtime_file).with_content(/^JENKINS_USERNAME='#{user}'$/)
      end

      it 'should escape the password' do
        should contain_file(agent_runtime_file).with_content(/^JENKINS_PASSWORD="#{password}"$/)
      end
    end

    describe 'with java_args as a string' do
      let(:args) { '-Xmx2g' }
      let(:params) do
        {
          :java_args => args
        }
      end

      it 'should set java_args' do
        should contain_file(agent_runtime_file).with_content(/^JAVA_ARGS="#{args}"$/)
      end
    end

    describe 'with java_args as an array' do
      let(:args) { ['-Xmx2g', '-Xms128m' ] }
      let(:params) do
        {
          :java_args => args
        }
      end

      it 'should convert java_args to a string' do
        args_as_string = args.join ' '
        should contain_file(agent_runtime_file).with_content(/^JAVA_ARGS="#{args_as_string}"$/)
      end
    end

    describe 'with LABELS as an array' do
      let(:params) do
        {
          :labels => ['hello', 'world']
        }
      end

      it 'should set LABEL as a string' do
        should contain_file(agent_runtime_file).with_content(/^LABELS="hello world"$/)
      end
    end

    describe 'with LABELS as a string' do
      let(:params) do
        {
          :labels => ['unlimited blades']
        }
      end

      it 'should set LABEL as a string' do
        should contain_file(agent_runtime_file).with_content(/^LABELS="unlimited blades"$/)
      end
    end
  end

  shared_examples 'using agent_name' do
    it { should contain_file(agent_runtime_file).with_content(/^CLIENT_NAME="jenkins-agent"$/) }
  end

  describe 'RedHat' do
    let(:facts) do
      {
        :osfamily                  => 'RedHat',
        :operatingsystem           => 'CentOS',
        :operatingsystemrelease    => '6.7',
        :operatingsystemmajrelease => '6',
        :kernel                    => 'Linux',

      }
    end
    let(:agent_runtime_file) { '/etc/sysconfig/jenkins-agent' }
    let(:agent_service_file) { '/etc/init.d/jenkins-agent' }
    it_behaves_like 'a jenkins::agent catalog'

    describe 'with agent_name' do
      let(:params) { { :agent_name => 'jenkins-agent' } }
      it_behaves_like 'using agent_name'
    end

    it { should_not contain_package('daemon') }

    context '::jenkins & ::jenkins::agent should co-exist' do
      let(:pre_condition) do
        <<-'EOS'
          include ::jenkins
          include ::jenkins::agent
        EOS
      end

      it { should_not raise_error }
    end

    describe 'with proxy_server' do
      let(:params) { { :proxy_server => 'https://foo' } }
      it do
        should contain_archive('get_swarm_client').with(
          :proxy_server => 'https://foo'
        )
      end
    end
  end

  describe 'Debian' do
    let(:facts) { { :osfamily => 'Debian', :lsbdistid => 'debian', :lsbdistcodename => 'natty', :operatingsystem => 'Debian', :kernel => 'Linux' } }
    let(:agent_runtime_file) { '/etc/default/jenkins-agent' }
    let(:agent_service_file) { '/etc/init.d/jenkins-agent' }

    it_behaves_like 'a jenkins::agent catalog'

    describe 'with agent_name' do
      let(:params) { { :agent_name => 'jenkins-agent' } }
      it_behaves_like 'using agent_name'
    end

    it do
      should contain_package('daemon')
        .that_comes_before('Service[jenkins-agent]')
    end
  end

  describe 'Darwin' do
    let(:facts) {
      {:osfamily => 'Darwin',
       :operatingsystem => 'Darwin',
       :kernel => 'Darwin'}
    }
    let(:home) { '/home/jenkins-agent' }
    let(:agent_runtime_file) { "#{home}/jenkins-agent" }
    let(:agent_service_file) { '/Library/LaunchDaemons/org.jenkins-ci.agent.jnlp.plist' }

    it_behaves_like 'a jenkins::agent catalog'

    # NOTE: pending because jenkins-agent doesn't get installed on Darwin
    describe 'with agent_name' do
      let(:params) { { :agent_name => 'jenkins-agent' } }
      it_behaves_like 'using agent_name'
    end

    it { should_not contain_package('daemon') }
  end

  describe 'Unknown' do
    let(:facts) { { :ostype => 'Unknown' } }
    it { expect { should raise_error(Puppet::Error) } }
  end
end
